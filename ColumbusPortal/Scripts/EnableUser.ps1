...
if($Confirm) {
    ...
    Enable-ADAccount -Identity ($UserPrincipalName -split '@')[0] -Server $Config.DomainFQDN -Credential $Cred
    ...
    try {
        ...
        if($mbx) {
            Set-Mailbox -HiddenFromAddressListsEnabled $false -Identity $UserPrincipalName
        }
        Else {
