...
function Get-Office365Information {
    ...
    if($Config.TenantID365.ID -eq $null){
        ...
        foreach($i in $SKU){
            ...
            $newid.PartnerName   = Get-MsolPartnerInformation -TenantId $Config.TenantID365.ID | select -ExpandProperty PartnerCompanyName
            $newid.TenantID      = $Config.TenantID365.ID
            $newid.Admin         = $Config.TenantID365.Admin
