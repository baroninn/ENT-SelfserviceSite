function New-ExchangeProxyModule {
    ...
    $server      = "cpo-exch-01.hosting.capto.dk"
    $sessionName = 'CaptoExchange'
    ...
    try {
        $returnModule = Import-PSSession -Session (New-PSsession -Name $sessionName -ConfigurationName Microsoft.Exchange -ConnectionUri "http://$server/Powershell") -CommandName $Command -AllowClobber
    }
    catch {
