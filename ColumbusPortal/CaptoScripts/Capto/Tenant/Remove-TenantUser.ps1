function Remove-TenantUser {
    ...
    Process {
        ...
        if ($Config.Product.Name -eq "Member2015") {
            ...
            if (-not (Test-Path $userFoldersPath)) {
                ...
                if ($PSCmdlet.ShouldProcess($userFoldersPath, "Remove user folder")) {
                    try {
                        robocopy C:\DontDelete "$userFoldersPath" /purge | out-null
            ...
            $UevProfilePath = Join-Path $Config.CitrixUevProfilePath ($user.SamAccountName + ".V2")
            if (-not (Test-Path $UevProfilePath)) {
                Write-Output "The path '$UevProfilePath' was not found"
            }
            else {
                if ($PSCmdlet.ShouldProcess($UevProfilePath, "Remove user folder")) {
                    try {
                        robocopy C:\DontDelete "$UevProfilePath" /purge | out-null
                        $UevProfilePath | Remove-Item -Recurse -Force
                    }
                    catch {
                        Write-Output "Unable to remove user folder '$UevProfilePath'. You must do this manually"
                    }
                }
            }
        }
