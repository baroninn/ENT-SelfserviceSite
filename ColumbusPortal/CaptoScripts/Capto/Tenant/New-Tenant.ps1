function New-Tenant {
    ...
    ## Create Customer File services from Capto
    if ($Config.Product.Name -eq "Legal") {
        $FSBlock = {
            param ($Name, $Config)

                C:\CaptoFS\Add-CaptoFSCustomer.ps1 -customer $Name -baseDir $Config.PublicDfsUncPath -shadowBaseDir $Config.VersionDfsUncPath -serviceConfigFileName "C:\CaptoFS\CaptoFS-config.json"
        }
        Invoke-Command -ComputerName $Config.FileServer.Fqdn -Credential $RemoteCredential -ScriptBlock $FSBlock -ArgumentList $Name, $Config -Authentication Credssp
    }
    ...
    elseif ($Product -eq 'Legal') {
        ...
        # CPO-WEB-01..
        Write-Verbose "Adding Constrained delegation to  $($Config.ServiceAccount.Name), $($Config.LegalWebServer.Name)"
        Get-ADComputer -Identity $Config.LegalWebServer.Name -Server $Config.DomainFQDN | Set-ADComputer -Add @{"msDS-AllowedToDelegateTo"=@("DynamicsNAV/$($Config.Name):7046", "DynamicsNAV/$($Config.Name).hosting.capto.dk:7046")}
        ...
        $xmlTemplate = Get-ChildItem -Path "\\hosting.capto.dk\data\systemhosting\TemplateFiles\ClientUserSettings.config" -Filter 'ClientUserSettings.config' -Recurse
        foreach($file in $xmlTemplate){
            ...
            $XMLTenant = Get-ChildItem -Path $config.CapLegalConfigFilePath -Filter 'ClientUserSettings.config' -Recurse
    ...
    if ($Product -eq 'AdvoPlus') {
        ...
        Write-Verbose "Product not AdvoPlus, Creating database in new way for Legal and up.."
        }
    ###
}