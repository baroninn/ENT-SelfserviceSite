function New-TenantConfig {
    ...
    $Config.FilehostingGroupName      = "$($Config.Name)_Access_FileHosting"
    ...
    $Config.LegalRoleGroupName        = "Base_Role_Legal2016User_MCS"
    $Config.CitrixUevProfilePath      = "\\capto\data\uev_profiles"
    ...
    $Config.Groups                   +=   New-GroupObject -Name $Config.FilehostingGroupName      -SamAccountName $Config.FilehostingGroupName      -GroupCategory Security -GroupScope Global      -Path $Config.AccessGroupsOU
    ...
    if($Product -eq "Legal"){
    $Config.LegalWebServer.Name = "CPO-WEB-01"
    $Config.LegalWebServer.Fqdn = "$($Config.LegalWebServer.Name).$($Config.DomainFQDN)"
    }
