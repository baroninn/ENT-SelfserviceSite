function New-LegalWebServiceInstance {
    ...
    Process {
        ...
        if ($ComputerName) {
            ...
            Invoke-Command -ComputerName $ComputerName -ScriptBlock $webServiceScriptBlock -ArgumentList $TenantName, $CompanyName, $Force, $Remove -Authentication Kerberos -Credential $RemoteCredential
        }
        else {
            $testPath    = $WebServicePath
            $testLogPath = $WebServiceLogPath
            Invoke-Command -ScriptBlock $webServiceScriptBlock -ArgumentList $TenantName, $CompanyName, $Force, $Remove -Authentication Kerberos -Credential $RemoteCredential
        }
        ...
        if(-not $Remove){
            ...
            New-Item -Path $tenantLogPath -ItemType Directory > $null
