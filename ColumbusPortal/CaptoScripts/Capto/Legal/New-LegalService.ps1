function New-LegalService {
    ...
    Process {
        ...
        $serviceArgumentList = "Create `"$serviceName`" binpath= `"$binPath `$$ServerInstance`" Type= share DisplayName= `"Microsoft Dynamics NAV Server [$ServerInstance]`" obj= `"$($ServiceAccount.UserName)`" password= `"$($ServiceAccount.GetNetworkCredential().Password)`" start= AUTO depend= HTTP/NetTcpPortSharing"
