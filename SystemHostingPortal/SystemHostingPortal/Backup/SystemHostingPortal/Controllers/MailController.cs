    ...
    public class MailController : Controller
    {
        ...
        public ActionResult SetCalendarPermissions(FormCollection _POST)
        {
            try
            {
                ...
                model.CalendarPermissions.Identity = _POST["identity"];
                model.CalendarPermissions.User = _POST.GetValues("user[]");
                model.CalendarPermissions.AccessRights = _POST.GetValues("accessrights[]");

                Common.Log(string.Format("has run Mail/SetCalendarPermissions for user {0}\\{1}", model.CalendarPermissions.Organization, model.CalendarPermissions.Identity));
        ...
        // ajax function - returns a <select> with the specified select name, and sets up a script to update the specified input field; also triggers the specified javascript function when a new input is selected
        public string GetMailboxes2(string organization, string selectname = "upnselector", string inputname = "userprincipalname", string triggerfunction = "")
        {
            if (triggerfunction.Contains(" ")) return "Error: Bad triggerfunction";

            string returnstr = "<select name=\"" + selectname + "\"><option selected=\"selected\">... or select a person</option>";
            ...
            returnstr += "<script>$(\"select[name=" + selectname + "]\").change(function () { var upn = $(\"select[name=" + selectname + "]\").val(); $(\"input[name=" + inputname + "]\").val(upn); " + triggerfunction + "; }); </script>";
