function New-AdvoPlusWebServiceInstance {
    [Cmdletbinding()]
    param (
    ...
    Process {
        ...
        $webServiceScriptBlock = {
            ...
            if ($webApp.Count -gt 0 -and -not $Force -and -not $Remove) {
                Write-Error "AdvoPlus Web Service has already been created for '$TenantName'."
            }
            ...
            if ($ComputerName) {
                ...
                Invoke-Command -ComputerName $ComputerName -ScriptBlock $webServiceScriptBlock -ArgumentList $ServerInstance, $WebServiceLogPath, $CompanyName, $Force, $Remove
            }
            else {
                $testPath    = $WebServicePath
                $testLogPath = $WebServiceLogPath
                Invoke-Command -ScriptBlock $webServiceScriptBlock -ArgumentList $ServerInstance, $WebServiceLogPath, $CompanyName, $Force, $Remove
            }
