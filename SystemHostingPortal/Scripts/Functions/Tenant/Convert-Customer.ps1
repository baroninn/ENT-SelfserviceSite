function Convert-Customer {
    ...
    try{Remove-ADGroup -Identity $Config.AdvoPlusRepoLogsGroupName -Server $Config.DomainFQDN -Confirm:$false}catch{}
    ...
    if ($Config.Product.Name -eq "AdvoPlus") {
        ...
        try {
            New-AdvoPlusMobileServerInstance -TenantName $Config.Name -CompanyName $CompanyName -ComputerName $Config.MobileServer.Fqdn -Remove $True
        }
        catch {
            Write-Error ($_)
        }
